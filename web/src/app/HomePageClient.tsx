...
export default function Home() {
  ...
  const oauthBootstrapRunRef = useRef(0);
  const oauthRetryTimeoutRef = useRef<number | null>(null);
  ...
  const clearOauthRetryTimeout = useCallback(() => {
    if (oauthRetryTimeoutRef.current === null) {
      return;
    }
    window.clearTimeout(oauthRetryTimeoutRef.current);
    oauthRetryTimeoutRef.current = null;
  }, []);

  const syncAuth = useCallback(() => {
    clearOauthRetryTimeout();
    const params = new URLSearchParams(window.location.search);
    const urlToken = params.get("token");
    const runId = oauthBootstrapRunRef.current + 1;
    oauthBootstrapRunRef.current = runId;

    const isActive = () => oauthBootstrapRunRef.current === runId;
        ...
        const attemptOAuthBootstrap = async (attempt: number) => {
          ...
          try {
            ...
            if (attempt < OAUTH_BOOTSTRAP_MAX_ATTEMPTS) {
              ...
              oauthRetryTimeoutRef.current = window.setTimeout(() => {
                if (!isActive()) {
                  return;
                }
    ...
    void run();
  }, [clearAuthState, clearOauthRetryTimeout, establishSession, syncStoredAuth, t.loginForm.networkError]);

  useEffect(() => {
    syncAuth();

    return () => {
      oauthBootstrapRunRef.current += 1;
      clearOauthRetryTimeout();
    };
  }, [clearOauthRetryTimeout, syncAuth]);
