...
async function recoverStaleDisconnectedAgentTasks(
  userId: string,
  tasks: TaskRecordForRecovery[],
): Promise<void> {
...
export async function GET(request: NextRequest) {
  ...
  if (recoverStale) {
    await recoverStaleDisconnectedAgentTasks(
      user.id,
      tasks as unknown as TaskRecordForRecovery[],
    );
  }
