...
describe("agent-gateway ownership handling", () => {
  ...
  it("marks AI task terminal exits final so chat does not keep sending to a daemon owner", async () => {
    vi.mocked(db.task.findFirst).mockResolvedValueOnce({
      id: "task-ai-terminal-exit-1",
      projectId: "proj-1",
      taskType: "ai_task",
      status: "running",
      agentHost: "daemon-a",
    ...
    await handleTerminalExitEvent({
      ...
      agentHost: "daemon-a",
      payload: {
        task_id: "task-ai-terminal-exit-1",
        pty_session_id: "ai-terminal:task-ai-terminal-exit-1",
        seq: 17,
        exit_code: 0,
        closed_at: "2026-03-11T09:00:00.000Z",
      },
    });

    expect(db.task.update).toHaveBeenCalledWith({
      where: { id: "task-ai-terminal-exit-1" },
      data: { status: "completed", executionHost: "daemon-a" },
    });
    ...
    expect(realtimeHub.broadcastTerminal).toHaveBeenCalledWith("user-1", "task-ai-terminal-exit-1", {
      type: "terminal_exit",
      payload: {
        task_id: "task-ai-terminal-exit-1",
        project_id: "proj-1",
  ...
  it("broadcasts AI task terminal errors without marking the task killed", async () => {
    vi.mocked(db.task.findFirst).mockResolvedValueOnce({
      id: "task-ai-terminal-err-1",
      projectId: "proj-1",
      taskType: "ai_task",
      status: "running",
      agentHost: "daemon-a",
      executionHost: "conductor-fire-task-ai-terminal-err-1",
      ptySession: null,
      taskStatusEvents: [],
    } as any);
    ...
    await handleTerminalErrorEvent({
      ...
      agentHost: "daemon-a",
      payload: {
        task_id: "task-ai-terminal-err-1",
        pty_session_id: "ai-terminal:task-ai-terminal-err-1",
        message: "AI terminal attach requires a session_id to resume",
      },
    });
    ...
    expect(realtimeHub.broadcastTerminal).toHaveBeenCalledWith("user-1", "task-ai-terminal-err-1", {
      type: "terminal_error",
      payload: {
        task_id: "task-ai-terminal-err-1",
        project_id: "proj-1",
  ...
  it("marks AI task terminal errors final when the daemon already stopped the fire process", async () => {
    vi.mocked(db.task.findFirst).mockResolvedValueOnce({
      id: "task-ai-terminal-spawn-err-1",
      projectId: "proj-1",
      taskType: "ai_task",
      status: "running",
      agentHost: "daemon-a",
    ...
    await handleTerminalErrorEvent({
      ...
      agentHost: "daemon-a",
      payload: {
        task_id: "task-ai-terminal-spawn-err-1",
        pty_session_id: "ai-terminal:task-ai-terminal-spawn-err-1",
        message: "node-pty spawn failed",
        task_status: "killed",
      },
    });

    expect(db.task.update).toHaveBeenCalledWith({
      where: { id: "task-ai-terminal-spawn-err-1" },
      data: { status: "killed", executionHost: "daemon-a" },
    });
    expect(realtimeHub.broadcast).toHaveBeenCalledWith("user-1", "proj-1", {
      type: "task_status_update",
      payload: {
        ...
        project_id: "proj-1",
        status: "killed",
        summary: "node-pty spawn failed",
      },
    });
    expect(realtimeHub.broadcastTerminal).toHaveBeenCalledWith("user-1", "task-ai-terminal-spawn-err-1", {
      type: "terminal_error",
      payload: {
        task_id: "task-ai-terminal-spawn-err-1",
        project_id: "proj-1",
