...
describe("Daemon", () => {
  ...
  it("creates PTY tasks, strips task-scoped env, and sends a terminal snapshot on fresh attach", async () => {
    ...
    try {
      ...
      assert.strictEqual(
        ...
        "project_path_validation,project_path_create,project_agents_registry,restart_daemon,refresh_session_inplace,restart_first_message,persistent_round_v1,task_attachments_v1,backend_session_list,custom_commands,update_daemon,remote_exec,remote_file,pty_task,terminal_snapshot",
      );
  ...
  it("disables PTY capability headers and rejects PTY tasks when startup self-check fails", async () => {
    ...
    assert.strictEqual(
      webSocketClientOptions.extraHeaders["x-conductor-capabilities"],
      "project_path_validation,project_path_create,project_agents_registry,restart_daemon,refresh_session_inplace,restart_first_message,persistent_round_v1,task_attachments_v1,backend_session_list,custom_commands,update_daemon,remote_exec,remote_file",
    );
