...
function windowsEnvPath(env, key) {
...
function listDirectoryNames(root, readdirSyncFn) {
  try {
...
function visualStudioGitCandidates(env, readdirSyncFn) {
  const roots = [
...
export function resolveConfiguredGitCommand(userConfig = {}, env = process.env) {
  return (
    stripWrappingQuotes(env.CONDUCTOR_GIT) ||
    stripWrappingQuotes(env.GIT_EXECUTABLE) ||
    stripWrappingQuotes(userConfig.git_path) ||
    stripWrappingQuotes(userConfig.gitPath) ||
    stripWrappingQuotes(userConfig.git_command) ||
    stripWrappingQuotes(userConfig.gitCommand) ||
    stripWrappingQuotes(userConfig.git?.path) ||
    stripWrappingQuotes(userConfig.git?.command)
  );
}
