...
async function main() {
  ...
  lines.push(
    ...
    "# remote_exec: true",
    "",
    "# Accept `conductor remote cp` file transfers for this host.",
    "# Env: CONDUCTOR_REMOTE_FILE",
    "# remote_file: true",
    "",
    ...
    "# ---------------------------------------------------------------------------",
    "",
    "# Optional: extra Codex auth.json files to switch between from the app's",
    "# account picker (shown only when more than one is listed). The file name",
    "# (without .json) becomes the account name. Without this, quota comes from",
